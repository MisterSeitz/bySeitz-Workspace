# 🛡️ Cybersecurity & AI News Intelligence Pipeline

This actor is a powerful data-gathering tool that transforms raw news from top RSS feeds (focused on Cybersecurity and AI) into structured, actionable intelligence. It uses **DuckDuckGo News Search** to gather real-time context and an LLM (OpenAI) to perform advanced analysis.

This actor is designed to run on a schedule, continuously gathering fresh intelligence. The structured data it produces is intended to be consumed by other systems, such as our flagship actor, **Content Blueprint AI**, which can use this stream of data to generate reports, threat briefings, analysis, and more.
...
2.  **As a Standalone Tool:** Run this actor to generate high-quality, structured news intelligence. You are free to download the resulting dataset for your own threat analysis, competitive reporting, or to feed into other custom security workflows.

---

## Features

-   **Comprehensive Source Aggregation**: Gathers news from a curated list of top-tier Cybersecurity, InfoSec, and AI feeds (e.g., CISA, Krebs on Security, The Hacker News).
...
-   **Advanced AI Analysis**: Leverages an LLM to analyze each article for sentiment/risk (e.g., `High Risk`, `Low Risk/Informational`), categorize the topic (e.g., `Vulnerability/CVE`, `Malware/Ransomware`), and extract key entities.
...
| :--- | :--- | :--- | :--- |
| `source` | String | `all` | The news source category to use (e.g., 'all', 'cisa-advisories', 'krebsonsecurity'). |
...
| `sentiment` | String | The AI-analyzed risk/impact (e.g., `High Risk`, `Low Risk/Informational`). |
| `category` | String | The AI-assigned category (e.g., `Vulnerability/CVE`, `Malware/Ransomware`). |
| `key_entities` | Array of Strings | A list of key entities like companies, vulnerabilities, or threat actors. |