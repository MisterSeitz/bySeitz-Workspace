...
async def rss_fetcher(state: WorkflowState) -> dict:
    ...
    for entry in rss_entries:
        article = Article(
            title=entry.title,
            # 💡 FIX: Cast entry.link to string to prevent Pydantic/LangGraph AttributeErrors
            url=str(entry.link), 
            source=entry.source,
...
async def process_and_save_article(state: WorkflowState) -> dict:
    ...
    if ai_overview:
        ...
        # Pass the runTestMode flag to the analysis function
...
async def main():
    ...
    async with Actor:
        ...
        Actor.log.info("Starting World News intelligence pipeline (2 Pay Points: Google Search AI Mode + LLM Analysis).")
        await app.ainvoke({
            "config": config,
            "articles": [],
            "processed_count": 0
