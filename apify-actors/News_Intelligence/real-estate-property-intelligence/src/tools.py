...
CATEGORIES = [
    "Residential Sales/Pricing", "Commercial/Office/Retail", "Investment/Financing/REITs",
    "Mortgage/Interest Rates", "Market Trends/Forecasts", "Property Tech (PropTech)",
    "Regulation/Policy/Zoning", "Luxury Market"
]
...
def fetch_rss_feeds(source: str, custom_url: str = None, max_articles: int = 10) -> List[RSSFeed]:
    feed_map = {
        ...
        "keepingcurrentmatters": "https://www.keepingcurrentmatters.com/feed/",
        "propertyweek": "https://www.propertyweek.com/feed",
        "inhabit_corcoran": "https://inhabit.corcoran.com/feed/",
        ...
        "agentfire": "https://agentfire.com/feed/",
        "brownstoner": "https://www.brownstoner.com/feed",
        "eliteagentmag": "http://feeds.feedburner.com/EliteAgentMag",
        "realtybiznews": "https://realtybiznews.com/feed",
        "realtytimes": "https://realtytimes.com/",
        "homelight": "https://www.homelight.com/blog/feed/",
        "americangenius": "https://theamericangenius.com/housing/feed",
        ...
        "flippingjunkie": "https://flippingjunkie.com/feed/",
        "custom": custom_url
    }
    urls = []
...
async def summarize_snippets_with_llm(snippets: str, is_test_mode: bool) -> str:
    ...
    client = init_openai()
    prompt = f"Synthesize a concise, neutral, one-paragraph summary of the main Real Estate or Property news event from the following search results. Note the different sources and dates, and **briefly mention any significant variations in their reporting (e.g., conflicting facts, different sentiment)**.\n\nSnippets:\n---\n{snippets}\n---"
    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo-0125",
            messages=[
                {"role": "system", "content": "You are a Real Estate and Property market news summarization assistant. Your goal is to synthesize a single, coherent paragraph from multiple sourced snippets. Base your summary *only* on the snippets. If you detect notable differences in reporting between sources, briefly mention it."},
...
async def fetch_summary_from_duckduckgo(
    ...
    if is_test_mode:
        ...
        sources = [{"title": "Test Source A", "url": "https://example.com/a", "source": "TestRealtyNews", "date": "2025-10-19"}]
...
async def analyze_article_summary(summary: str, is_test_mode: bool) -> Dict[str, Any]:
    if is_test_mode:
        ...
        return {"sentiment": "High Growth (TEST)", "category": "Residential Sales/Pricing (TEST)", "key_entities": ["Zillow", "New York City", "30-Year Fixed Rate"]}
    ...
    sentiment_options = ["High Growth", "Cooling Market", "Policy Driven", "Informational"]
    category_list_str = ", ".join(CATEGORIES)
    prompt = f'Analyze the following Real Estate and Property market news summary: "{summary}"\n\nBased ONLY on the summary, provide a structured JSON output with:\n1. sentiment: The market dynamic or impact level ({", ".join(sentiment_options)}).\n2. category: The best category from this list: {category_list_str}.\n3. key_entities: A list of up to 3 key cities, regions, companies, or financial metrics mentioned.\n\nOutput a single valid JSON object.'

    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo-0125",
            messages=[
                {"role": "system", "content": "You are a professional Real Estate and Property market analyst. Return a JSON object with 'sentiment', 'category', and 'key_entities'."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.0,
