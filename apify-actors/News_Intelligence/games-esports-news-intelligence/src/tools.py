...
CATEGORIES = [
    "Game Review/Score", "Tournament Results/Recap", "Esports Business/Sponsorship",
    "Game Announcement/Trailer", "Patch Notes/Update", "Studio News/Acquisition",
    "Hardware/Tech", "Informational/General"
]
...
def fetch_rss_feeds(source: str, custom_url: str = None, max_articles: int = 10) -> List[RSSFeed]:
    feed_map = {
        ...
        "gamerant": "https://gamerant.com/feed/gaming/",
        "ign-articles": "https://www.ign.com/rss/v2/articles/feed",
        "ign-wikis": "https://www.ign.com/rss/wikis/feed",
        ...
        "gamespot-newgames": "https://www.gamespot.com/feeds/new-games",
        "kotaku": "https://kotaku.com/feed",
        "escharts": "https://escharts.com/feed",
        "esportsgg": "http://esports.gg/api/feed",
        "vgc-news": "https://www.videogameschronicle.com/category/news/feed/",
        "vgc-review": "https://www.videogameschronicle.com/category/review/feed/",
        "vgc-features": "https://www.videogameschronicle.com/category/features/feed/",
        "gamesradar": "https://www.gamesradar.com/feeds.xml",
        "gamesindustrybiz": "https://www.gamesindustry.biz/feed",
        "thegamer": "https://www.thegamer.com/feed/",
        "gamedeveloper": "https://www.gamedeveloper.com/rss.xml",
        "instantgaming": "https://news.instant-gaming.com/en/rss.xml",
        "n4g": "https://n4g.com/rss/news",
        "insidergaming": "https://insider-gaming.com/category/news/feed/",
        "custom": custom_url
    }
    urls = []
...
async def summarize_snippets_with_llm(snippets: str, is_test_mode: bool) -> str:
    if is_test_mode: return "This is a test summary generated from dummy search snippets. It notes that Source A reported a new game's high review score while Source B focused on the game's competitive multiplayer meta, showcasing a variation in reporting."

    client = init_openai()
    prompt = f"Synthesize a concise, neutral, one-paragraph summary of the main Gaming or Esports news event from the following search results. Note the different sources and dates, and **briefly mention any significant variations in their reporting (e.g., conflicting facts, different sentiment)**.\n\nSnippets:\n---\n{snippets}\n---"
    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo-0125",
            messages=[
                {"role": "system", "content": "You are a Gaming and Esports news summarization assistant. Your goal is to synthesize a single, coherent paragraph from multiple sourced snippets. Base your summary *only* on the snippets. If you detect notable differences in reporting between sources, briefly mention it."},
...
async def fetch_summary_from_duckduckgo(
    ...
    if is_test_mode:
        ...
        sources = [{"title": "Test Source A", "url": "https://example.com/a", "source": "TestGameNews", "date": "2025-10-19"}]
...
async def analyze_article_summary(summary: str, is_test_mode: bool) -> Dict[str, Any]:
    if is_test_mode:
        ...
        return {"sentiment": "High Hype (TEST)", "category": "Game Announcement/Trailer (TEST)", "key_entities": ["Final Fantasy 17", "Square Enix", "PlayStation 6"]}
    ...
    sentiment_options = ["High Hype", "Moderate Interest", "Informational"]
    category_list_str = ", ".join(CATEGORIES)
    prompt = f'Analyze the following Gaming and Esports news summary: "{summary}"\n\nBased ONLY on the summary, provide a structured JSON output with:\n1. sentiment: The impact or interest level ({", ".join(sentiment_options)}).\n2. category: The best category from this list: {category_list_str}.\n3. key_entities: A list of up to 3 key games, teams, companies, or people mentioned.\n\nOutput a single valid JSON object.'

    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo-0125",
            messages=[
                {"role": "system", "content": "You are a professional Gaming and Esports analyst. Return a JSON object with 'sentiment', 'category', and 'key_entities'."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.0,
