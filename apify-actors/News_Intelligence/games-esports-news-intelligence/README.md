...
This actor is a powerful data-gathering tool that transforms raw news from top RSS feeds (focused on **Video Games, Esports, and Gaming Culture**) into structured, actionable intelligence. It uses DuckDuckGo News Search to gather real-time context and an LLM (OpenAI) to perform advanced analysis.

This actor is designed to run on a schedule, continuously gathering fresh intelligence. The structured data it produces is intended to be consumed by other systems, such as our flagship actor, **Content Blueprint AI**, which can use this stream of data to generate reports, game briefings, analysis, and more.
...
2.  **As a Standalone Tool:** Run this actor to generate high-quality, structured news intelligence. You are free to download the resulting dataset for your own competitive analysis, market research, or to feed into other custom gaming workflows.

---

## Features

-   **Comprehensive Source Aggregation**: Gathers news from a curated list of top-tier Gaming and Esports feeds (e.g., IGN, PC Gamer, Esports Insider).
...
-   **Advanced AI Analysis**: Leverages an LLM to analyze each article for sentiment/hype (e.g., `High Hype`, `Moderate Interest`, `Informational`), categorize the topic (e.g., `Game Review/Score`, `Tournament Results/Recap`, `Studio News/Acquisition`), and extract key entities.
...
| :--- | :--- | :--- | :--- |
| `source` | String | `all` | The news source category to use (e.g., 'all', 'ign-articles', 'pcgamer'). |
...
| :--- | :--- | :--- |
| `source` | String | The name of the news source (e.g., 'IGN Articles'). |
...
| `sentiment` | String | The AI-analyzed impact/interest level (e.g., `High Hype`, `Moderate Interest`). |
| `category` | String | The AI-assigned category (e.g., `Game Review/Score`, `Tournament Results/Recap`). |
| `key_entities` | Array of Strings | A list of key entities like games, teams, companies, or people.