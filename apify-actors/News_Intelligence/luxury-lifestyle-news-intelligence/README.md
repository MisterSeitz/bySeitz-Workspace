# 💎 Luxury & Lifestyle News Intelligence Pipeline

This actor is a powerful data-gathering tool that transforms raw news from top RSS feeds (focused on **Luxury, High-End Market, and Lifestyle**) into structured, actionable intelligence. It uses DuckDuckGo News Search to gather real-time context and an LLM (OpenAI) to perform advanced analysis.
...
2.  **As a Standalone Tool:** Run this actor to generate high-quality, structured news intelligence. You are free to download the resulting dataset for your own market analysis, brand research, or to feed into other custom luxury workflows.

---

## Features

-   **Comprehensive Source Aggregation**: Gathers news from a curated list of top-tier Luxury and Lifestyle feeds (e.g., Robb Report, Luxuo, Luxury Daily).
...
-   **Advanced AI Analysis**: Leverages an LLM to analyze each article for sentiment/market dynamic (e.g., `Brand Growth`, `Market Downturn`, `Acquisition/Partnership`), categorize the topic (e.g., `Luxury Retail/Apparel`, `Automotive/Yachts/Aviation`, `High-End Real Estate/Design`), and extract key entities.
...
| :--- | :--- | :--- | :--- |
| `source` | String | `all` | The news source category to use (e.g., 'all', 'robbreport_main', 'luxurydaily'). |
...
| :--- | :--- | :--- |
| `source` | String | The name of the news source (e.g., 'Robb Report (Main)'). |
...
| `sentiment` | String | The AI-analyzed market dynamic (e.g., `Brand Growth`, `Market Downturn`). |
| `category` | String | The AI-assigned category (e.g., `Luxury Retail/Apparel`, `Automotive/Yachts/Aviation`). |
| `key_entities` | Array of Strings | A list of key entities like brands, events, companies, or people.