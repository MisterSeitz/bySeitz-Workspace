...
# --- AMENDED CATEGORIES FOR WEB3/DECENTRALIZED TECH ---
CATEGORIES = [
    "DeFi/Lending/Staking", "NFTs/Gaming/Metaverse", "Blockchain Infrastructure/Layer 1 & 2",
    "Regulation/Policy/CBDCs", "Venture Capital/Funding", "Security/Hacks/Audits",
    "Decentralized Autonomous Organizations (DAOs)", "Cross-Chain/Interoperability"
]

def fetch_rss_feeds(source: str, custom_url: str = None, max_articles: int = 10) -> List[RSSFeed]:
    # --- AMENDED FEED MAP FOR WEB3/DECENTRALIZED TECH ---
    feed_map = {
...
async def summarize_snippets_with_llm(snippets: str, is_test_mode: bool) -> str:
    ...
    client = init_openai()
    # --- AMENDED PROMPT FOR WEB3/DECENTRALIZED TECH ---
    prompt = f"Synthesize a concise, neutral, one-paragraph summary of the main Web3, Crypto, or Decentralized Tech news event from the following search results. Note the different sources and dates, and **briefly mention any significant variations in their reporting (e.g., conflicting facts, different focus, or opposing perspectives)**.\n\nSnippets:\n---\n{snippets}\n---"
    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo-0125",
            messages=[
                {"role": "system", "content": "You are a Web3, Crypto, and Decentralized Technology news summarization assistant. Your goal is to synthesize a single, coherent paragraph from multiple sourced snippets. Base your summary *only* on the snippets. If you detect notable differences in reporting between sources, briefly mention it."},
...
async def fetch_summary_from_duckduckgo(
    ...
    if is_test_mode:
        ...
        sources = [{"title": "Test Source A", "url": "https://example.com/a", "source": "TestCoinDesk", "date": "2025-10-19"}]
...
async def analyze_article_summary(summary: str, is_test_mode: bool) -> Dict[str, Any]:
    if is_test_mode:
        # --- AMENDED TEST RESPONSE FOR WEB3/DECENTRALIZED TECH ---
        Actor.log.warning("ADMIN TEST MODE: Bypassing LLM analysis call.")
        return {"sentiment": "Bullish Trend (TEST)", "category": "DeFi/Lending/Staking (TEST)", "key_entities": ["Solana", "SEC", "Decentraland"]}
        # -----------------------------------------------
    ...
    # --- AMENDED SENTIMENT OPTIONS FOR WEB3/DECENTRALIZED TECH ---
    sentiment_options = ["Bullish Trend", "Bearish Outlook", "Regulatory Focus", "Informational"]
    category_list_str = ", ".join(CATEGORIES)
    prompt = f'Analyze the following Web3 and Decentralized Tech news summary: "{summary}"\n\nBased ONLY on the summary, provide a structured JSON output with:\n1. sentiment: The market dynamic or impact level ({", ".join(sentiment_options)}).\n2. category: The best category from this list: {category_list_str}.\n3. key_entities: A list of up to 3 key protocols, companies, policies, or financial metrics mentioned.\n\nOutput a single valid JSON object.'

    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo-0125",
            messages=[
                {"role": "system", "content": "You are a professional Web3, Crypto, and Decentralized Technology market analyst. Return a JSON object with 'sentiment', 'category', and 'key_entities'."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.0,
