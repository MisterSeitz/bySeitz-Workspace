...
This actor is a powerful data-gathering tool that transforms raw news from top RSS feeds (focused on **Web3, Crypto, and Decentralized Technology**) into structured, actionable intelligence. It uses DuckDuckGo News Search to gather real-time context and an LLM (OpenAI) to perform advanced analysis.
...
2.  **As a Standalone Tool:** Run this actor to generate high-quality, structured news intelligence. You are free to download the resulting dataset for your own market analysis, investment research, or to feed into other custom Web3 workflows.

---

## Features

-   **Comprehensive Source Aggregation**: Gathers news from a curated list of top-tier Web3 and Crypto feeds (e.g., CoinDesk, Decrypt, The Defiant).
...
-   **Advanced AI Analysis**: Leverages an LLM to analyze each article for sentiment/market dynamic (e.g., `Bullish Trend`, `Bearish Outlook`, `Regulatory Focus`), categorize the topic (e.g., `DeFi/Lending/Staking`, `NFTs/Gaming/Metaverse`, `Regulation/Policy/CBDCs`), and extract key entities.
...
| :--- | :--- | :--- | :--- |
| `source` | String | `all` | The news source category to use (e.g., 'all', 'coindesk', 'thedefiant'). |
...
| :--- | :--- | :--- |
| `source` | String | The name of the news source (e.g., 'CoinDesk'). |
...
| `sentiment` | String | The AI-analyzed market dynamic (e.g., `Bullish Trend`, `Regulatory Focus`). |
| `category` | String | The AI-assigned category (e.g., `DeFi/Lending/Staking`, `Regulation/Policy/CBDCs`). |
| `key_entities` | Array of Strings | A list of key entities like protocols, companies, policies, or financial metrics.