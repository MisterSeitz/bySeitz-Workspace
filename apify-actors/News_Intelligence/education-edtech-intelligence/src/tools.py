...
# --- AMENDED CATEGORIES FOR EDUCATION/EDTECH ---
CATEGORIES = [
    "K-12 Policy/Funding", "Higher Education Trends", "EdTech/Learning Platforms",
    "Teacher/Faculty Development", "Curriculum/Pedagogy", "AI in Education",
    "Student Mental Health/Wellness", "Online/Blended Learning"
]

def fetch_rss_feeds(source: str, custom_url: str = None, max_articles: int = 10) -> List[RSSFeed]:
    # --- AMENDED FEED MAP FOR EDUCATION/EDTECH ---
    feed_map = {
...
async def summarize_snippets_with_llm(snippets: str, is_test_mode: bool) -> str:
    ...
    client = init_openai()
    # --- AMENDED PROMPT FOR EDUCATION/EDTECH ---
    prompt = f"Synthesize a concise, neutral, one-paragraph summary of the main Education or EdTech news event from the following search results. Note the different sources and dates, and **briefly mention any significant variations in their reporting (e.g., conflicting facts, different focus, or opposing perspectives)**.\n\nSnippets:\n---\n{snippets}\n---"
    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo-0125",
            messages=[
                {"role": "system", "content": "You are an Education and EdTech news summarization assistant. Your goal is to synthesize a single, coherent paragraph from multiple sourced snippets. Base your summary *only* on the snippets. If you detect notable differences in reporting between sources, briefly mention it."},
...
async def fetch_summary_from_duckduckgo(
    ...
    if is_test_mode:
        ...
        sources = [{"title": "Test Source A", "url": "https://example.com/a", "source": "TestEdTechReview", "date": "2025-10-19"}]
...
async def analyze_article_summary(summary: str, is_test_mode: bool) -> Dict[str, Any]:
    if is_test_mode:
        # --- AMENDED TEST RESPONSE FOR EDUCATION/EDTECH ---
        Actor.log.warning("ADMIN TEST MODE: Bypassing LLM analysis call.")
        return {"sentiment": "Innovation/Adoption (TEST)", "category": "EdTech/Learning Platforms (TEST)", "key_entities": ["Khan Academy", "California School Board", "AI Tutoring"]}
        # -----------------------------------------------
    ...
    # --- AMENDED SENTIMENT OPTIONS FOR EDUCATION/EDTECH ---
    sentiment_options = ["Innovation/Adoption", "Policy/Regulation", "Funding/Investment", "Informational"]
    category_list_str = ", ".join(CATEGORIES)
    prompt = f'Analyze the following Education and EdTech news summary: "{summary}"\n\nBased ONLY on the summary, provide a structured JSON output with:\n1. sentiment: The sector dynamic or impact level ({", ".join(sentiment_options)}).\n2. category: The best category from this list: {category_list_str}.\n3. key_entities: A list of up to 3 key companies, topics, policies, or institutions mentioned.\n\nOutput a single valid JSON object.'

    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo-0125",
            messages=[
                {"role": "system", "content": "You are a professional Education and EdTech market analyst. Return a JSON object with 'sentiment', 'category', and 'key_entities'."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.0,
