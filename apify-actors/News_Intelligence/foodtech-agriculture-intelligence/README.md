# 🌾 FoodTech & Agriculture Intelligence Pipeline

This actor is a powerful data-gathering tool that transforms raw news from top RSS feeds (focused on **FoodTech, AgTech, and Global Agriculture**) into structured, actionable intelligence. It uses DuckDuckGo News Search to gather real-time context and an LLM (OpenAI) to perform advanced analysis.
...
2.  **As a Standalone Tool:** Run this actor to generate high-quality, structured news intelligence. You are free to download the resulting dataset for your own market analysis, investment research, or to feed into other custom AgTech workflows.

---

## Features

-   **Comprehensive Source Aggregation**: Gathers news from a curated list of top-tier FoodTech and Agriculture feeds (e.g., AgFunderNews, FoodDive, Precision Farming Dealer).
...
-   **Advanced AI Analysis**: Leverages an LLM to analyze each article for sentiment/market dynamic (e.g., `Investment/Growth`, `Regulatory/Policy Change`, `Innovation/Adoption`), categorize the topic (e.g., `Precision/AgriTech`, `Alternative Proteins/Cell-Based`, `Supply Chain/Logistics`), and extract key entities.
...
| :--- | :--- | :--- | :--- |
| `source` | String | `all` | The news source category to use (e.g., 'all', 'agfundernews', 'fooddive'). |
...
| :--- | :--- | :--- |
| `source` | String | The name of the news source (e.g., 'AgFunderNews'). |
...
| `sentiment` | String | The AI-analyzed market dynamic (e.g., `Investment/Growth`, `Regulatory/Policy Change`). |
| `category` | String | The AI-assigned category (e.g., `Precision/AgriTech`, `Alternative Proteins/Cell-Based`). |
| `key_entities` | Array of Strings | A list of key entities like companies, commodities, policies, or technologies.